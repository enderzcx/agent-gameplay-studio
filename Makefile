...
.DEFAULT_GOAL := help
.PHONY: help check test-checker test-audit test-subtitles test-assembly test-tts test-production recorder recorder-offline check-examples clean
...
check: test-checker test-audit test-subtitles test-assembly test-tts test-production  ## Run every offline test (no network, no keys, no game)
	@echo
...
test-subtitles:  ## Subtitle paging + ASS time base (phrase paging, carry, PlayRes)
	$(PY) tests/test_subtitles.py

test-assembly:  ## EDL assembler smoke test on synthetic media (real ffmpeg; burn + source audio)
...
test-production:  ## gameplay-production journal: capture gate, evidence binding, resume-after-review
	$(PY) tests/test_production_run.py
