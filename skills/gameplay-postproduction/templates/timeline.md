...
| `audio_offset_s` | 旁白行 | 该段配音在画面窗口内**真实的**起点偏移（不是"一般 ~0.2s"）。声段 = `final 起点 + 本列` 起、加实测声长；静默与字幕对齐都按它算 |
...
| `hold_burned_in` | `freeze>0` | `yes` = 制作路径**真的**把保持标注画到了画面上（`build_sample.sh` 用 drawtext 烧录）。只在表格里写 `hold_mark` 不算；填 `no` 会被拒 |
...
字幕/音轨/成片绑在同一次制作上，并记录**实际读过的源视频与 EDL**）。只读旧媒体再手写一份声明，
不算验证。

**recipe 与 adopted**：首次制作之前，三份输出摘要根本还不存在，所以**输入 recipe 允许占位**
（`@PENDING@` / `-`）。制作路径跑完会把实测结果写回一份**新的** `adopted-timeline.tsv`——
占位被真实摘要填实、补上真实 `audio_offset_s` 与 `hold_burned_in`——**adopted 输出不允许占位**。
交付给 `ready` 的是这份 adopted 版，不是调用方手改过的输入。
