...
| **Silence is measured on the real audio, not the picture.** A narration occupies `[final_start, final_start + measured audio_duration_s]`; a 1 s line on a 66 s shot leaves 65 s of silence, and the narration ratio uses the same audio seconds | `check_timeline_audit.py audit` | exit `1`, naming the unjustified stretch |
...
| **One version, bound by content — not by counts.** The timeline declares `subtitle_sha256` / `audio_sha256` / `final_sha256`; the audit checks those digests, the per-cue subtitle text *and* its timing against each row's audio span, that `--audio` really carries an audio stream, and that the cut is the same total length as the timeline. Same segment count with re-worded or re-timed subtitles, an older audio track or an older export all fail | `check_timeline_audit.py audit` | exit `1`, naming which binding broke |
