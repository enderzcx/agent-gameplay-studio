...
| **Silence is measured on the real audio, not the picture.** A narration occupies `[final_start + audio_offset_s, + measured audio_duration_s]` (merged as a union, same basis as the ratio); a 1 s line on a 66 s shot leaves 65 s of silence, and the narration ratio uses the same audio seconds | `check_timeline_audit.py audit` | exit `1`, naming the unjustified stretch |
| **Long silence is justified per stretch.** Every narration gap ≥ threshold needs one ledger row (`keep` / `cut` / `narration_added` + a substantive reason); a stale row, or a ledger computed from picture windows, is an error | `check_timeline_audit.py audit` + `templates/silence-ledger.md` | exit `1`, listing the unjustified gap or the stale row |
| **Cross-phase lines must be declared, and order is not a timeline.** `changing phase` requires `claim_mode=retrospective`; the ordinal comparison only applies inside one event ("the picture has not reached that phase yet but the line already gives the result") | `check_timeline_audit.py audit` | exit `1`; declared retrospectives are listed in `phase_crossings` and marked unverified |
| **One version, bound by content — not by counts.** The timeline declares `subtitle_sha256` / `audio_sha256` / `final_sha256`; the audit checks those digests, the per-cue subtitle text *and* a timing that MATCHES each row's audio span (a sentence squeezed into the last 0.1 s fails), that `--audio` really carries an audio stream, and the `source_video`/`edl` binding recorded by the producer, and that the cut is the same total length as the timeline. Same segment count with re-worded or re-timed subtitles, an older audio track or an older export all fail | `check_timeline_audit.py audit` | exit `1`, naming which binding broke |
| **One production, not just four matching digests.** `produce-receipt.json` is written by the production path at the moment of production and binds the timeline plus subtitle/audio/cut to that one event; the audit cross-checks it and the recorded source digests. A receipt for a different timeline, or old media documented by a fresh declaration, fails | `check_timeline_audit.py audit`, `tools/voice/build_sample.sh` | exit `1` / exit `3`; the receipt is explicitly *unsigned*, so it is a consistency record, not an authenticity proof |
...
| **The first build closes the loop.** The input recipe may carry placeholder output digests; the production path cross-checks it against the real EDL (order, source ranges, 1x speed, freeze, narration text, measured audio length, offset) and the real `SRC_VIDEO` digest, then writes `adopted-timeline.tsv` with the real digests, the real `audio_offset_s` and `hold_burned_in`. `ready` consumes the adopted file; no second render to obtain digests | `tools/voice/produce_timeline.py` + `build_sample.sh` | unsupported form -> exit `3` + `DRAFT.txt` |
| **A hold mark must reach the picture.** `hold_burned_in: yes` is required for any freeze, and only the frame the assembler can actually freeze (the last frame of the shot) is accepted; otherwise it fails closed | `produce_timeline.py` + `check_timeline_audit.py` | exit `3` / exit `1` |
...
| `audit` | timeline TSV + preflight ledger + silence ledger + subtitle + audio + final cut + produce receipt | **the adopted timeline is self-consistent** — declared anchors cross-checked against each shot's real span, phase order, labelled holds inside their visible window, narration fits its window, per-gap silence justification, content-level version binding (subtitle text/timing + three digests), no stale manifest. |
...
`ready` **requires** `--final-mp4`, `--timeline`, `--preflight`, `--silence-ledger`, `--subtitle`,
`--audio` and `--receipt`; omitting any of them reports the state as `unverified`. It also **re-runs the audit
