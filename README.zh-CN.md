...
make help
make check          # 5 套离线测试：检查器 + 时间线审计 + 字幕分页与时基 + 组装/烧字幕/原声 + TTS 硬保证
...
| 字幕分页与时基 | **已验证 · 离线** | `tests/test_subtitles.py`：5 例——短语分页严丝合缝铺满时间窗、ASS 总厘秒进位、PlayRes = 视频尺寸、转义 |
| 合成媒体走 EDL 组装（真 `ffmpeg`） | **已验证 · 离线** | `tests/test_build_sample.py`：13 例，lavfi 合成素材（含首次一次 build 闭环、A/B 源错配、定格 fail-closed、libass 烧字幕 + 像素抽检、错带必须被拒、原声混音与无声源拒绝） |
